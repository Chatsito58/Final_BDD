...
class MainView(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.setWindowTitle("Sistema de Alquiler de Vehículos")
        ...
        self._role = role
        self._db_manager = db_manager if db_manager is not None else TripleDBManager()
        self._auth_manager = (
            auth_manager if auth_manager is not None else AuthManager(self._db_manager)
        )
        ...
        if (
            self.menu_admin
            and self.menu_admin.actions()
            and self.logout_action not in self.menu_admin.actions()
        ):
            ...
            if hasattr(self._db_manager, "set_on_reconnect_callback"):
                ...
                def mostrar_alerta_reconexion():
                    ...
                    QMessageBox.information(
                        self,
                        "Reconexión exitosa",
                        "Tu conexión con el servidor principal regresó, ahora se sincronizarán los cambios hechos en modo offline.",
                    )

                self._db_manager.set_on_reconnect_callback(mostrar_alerta_reconexion)
        except Exception as e:
            print(
                f"[SYNC][ERROR] No se pudo registrar el callback de desconexión en MainView: {e}"
            )
        ...
        if tab_principal is not None:
            ...
            if layout_principal is not None:
                ...
                self.btn_logout_tab.setStyleSheet(
                    "background-color: #3A86FF; color: white; border-radius: 8px; padding: 8px 0px; font-size: 15px;"
                )
        ...
        self.tabCambiarContrasena = self.findChild(
            QtWidgets.QWidget, "tabCambiarContrasena"
        )
        if self.tabCambiarContrasena is not None:
            ...
            if self.verticalLayoutTabCambiarContrasena is None:
                self.verticalLayoutTabCambiarContrasena = QVBoxLayout(
                    self.tabCambiarContrasena
                )
        else:
        ...
        if self.tabCambiarContrasena and self.verticalLayoutTabCambiarContrasena:
            try:
                ...
                QMessageBox.critical(
                    self,
                    "Error de UI",
                    f"Error al inicializar la pestaña de cambio de contraseña:\n{e}\n{tb}",
                )
        else:
            from PyQt5.QtWidgets import QMessageBox
            ...
            QMessageBox.critical(
                self,
                "Error de UI",
                "No se pudo inicializar la pestaña de cambio de contraseña. Por favor revisa el archivo main_window.ui y los nombres de los layouts.",
            )
    ...
    def _update_status_bar(self):
        r1 = getattr(
            self._db_manager,
            "is_remote1_active",
            lambda: getattr(self._db_manager, "remote1_active", False),
        )()
        r2 = getattr(
            self._db_manager,
            "is_remote2_active",
            lambda: getattr(self._db_manager, "remote2_active", False),
        )()
    ...
    def _cambiar_contrasena(self):
        ...
        logger.info(
            f"[DEBUG] _cambiar_contrasena llamado para usuario: {self._username}"
        )
        ...
        logger.info(
            f"[DEBUG] Llamando a self._auth_manager.cambiar_contrasena({self._username}, actual, nueva)"
        )
...
class AdminViewQt(MainView):
    def __init__(
        self, username, parent=None, db_manager=None, auth_manager=None, app=None
    ):
        super().__init__(
            username,
            "admin",
