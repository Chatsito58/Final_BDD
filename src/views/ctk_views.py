...
class ClienteView(BaseCTKView):
    ...
    def _build_tab_mis_reservas(self, parent):
        ...
        # Frame interno con ancho máximo y centrado
        scrollable_frame = ctk.CTkFrame(canvas, fg_color="#2A2D35")
        scrollable_frame_id = canvas.create_window(
            (0, 0), window=scrollable_frame, anchor="nw"
        )

        # Limitar el ancho máximo del frame interno
        def _resize_inner(event):
            canvas_width = min(event.width, 1100)
        ...
        # Frame de contenido (tarjetas)
        inner = ctk.CTkFrame(scrollable_frame)
        inner.pack(pady=20, padx=10)
        inner.grid_columnconfigure((0, 1, 2), weight=1)
        # Título usando grid
        ctk.CTkLabel(inner, text="Mis reservas", font=("Arial", 20, "bold")).grid(
    ...
    def _cargar_reservas_cliente(self, id_cliente):
        ...
        for r in reservas:
            ...
            ctk.CTkLabel(
                header, text=f"{modelo} ({placa})", font=("Arial", 14, "bold")
            ...
            fechas.pack(fill="x")
            ctk.CTkLabel(
                fechas, text=f"Salida: {salida:%Y-%m-%d %H:%M}", font=("Arial", 12)
            ).pack(side="left", padx=8)
            ctk.CTkLabel(
                fechas, text=f"Entrada: {entrada:%Y-%m-%d %H:%M}", font=("Arial", 12)
            ).pack(side="left", padx=8)
            # Montos
            montos = ctk.CTkFrame(card, fg_color="white")
            montos.pack(fill="x")
            ctk.CTkLabel(montos, text=f"Total: ${valor:,.0f}", font=("Arial", 12)).pack(
                side="left", padx=8
            )
            ctk.CTkLabel(
                montos, text=f"Abonado: ${abono:,.0f}", font=("Arial", 12)
            ).pack(side="left", padx=8)
            ctk.CTkLabel(montos, text=f"Saldo: ${saldo:,.0f}", font=("Arial", 12)).pack(
                side="left", padx=8
            )
            ...
            if seguro and costo_seguro:
                ctk.CTkLabel(
                    card,
                    text=f"Seguro: {seguro} (${costo_seguro:,.0f})",
                    font=("Arial", 11),
                    text_color="#1976D2",
            ...
            if desc and val_desc:
                ctk.CTkLabel(
                    card,
                    text=f"Descuento: {desc} (-${val_desc:,.0f})",
                    font=("Arial", 11),
                    text_color="#388E3C",
                ).pack(anchor="w", padx=12)
            ...
            if "pendiente" in estado_str.lower():
                ...
                ctk.CTkButton(
                    ...
                    command=lambda rid=id_reserva: self._cancelar_reserva_card(rid),
                ).pack(side="left", padx=8)
                ctk.CTkButton(
                    btns,
                    text="Editar fechas",
                    command=lambda rid=id_reserva: self._editar_reserva_card(rid),
                ).pack(side="left", padx=8)
    ...
    def _cancelar_reserva_card(self, id_reserva):
        ...
        if estado and estado[0][0] != 1:  # Solo permite cancelar si está 'Pendiente'
            messagebox.showwarning(
                "Aviso", "Solo puedes cancelar reservas en estado 'Pendiente'."
            )
            return
    ...
    def _editar_reserva_card(self, id_reserva):
        ...
        ctk.CTkLabel(
            win, text="Editar Fechas de Reserva", font=("Arial", 16, "bold")
        ).pack(pady=10)
        ...
        # Fecha y hora salida
        ctk.CTkLabel(
            fechas_frame, text="Nueva fecha y hora salida:", font=("Arial", 12, "bold")
        ...
        ctk.CTkLabel(
            fechas_frame, text="Nueva fecha y hora entrada:", font=("Arial", 12, "bold")
        ).pack(anchor="w", pady=5)
        ...
        nuevo_valor_label = ctk.CTkLabel(
            win,
            text=f"Valor actual: ${valor_actual:,.0f}",
            font=("Arial", 12, "bold"),
            text_color="#FFD700",
        )
        ...
        def calcular_nuevo_valor(*args):
            try:
                ...
                if fecha_entrada_dt <= fecha_salida_dt:
                    nuevo_valor_label.configure(
                        text="Error: Fecha entrada debe ser posterior a salida"
                    )
                    return
                ...
                tarifa_query = """
                    ...
                    WHERE v.placa = %s
                """
                tarifa_result = self.db_manager.execute_query(
                    tarifa_query, (id_vehiculo,)
                )
                if not tarifa_result:
                    nuevo_valor_label.configure(
                        text="Error: No se pudo obtener la tarifa"
                    )
                    return
                ...
                if id_seguro:
                    seguro_query = (
                        "SELECT costo FROM Seguro_alquiler WHERE id_seguro = %s"
                    )
        ...
        def guardar():
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", f"No se pudo actualizar la reserva: {exc}"
                )
    ...
    def _registrar_abono(self, id_reserva, monto, metodo, referencia=None):
        ...
        try:
            ...
            """
            self.db_manager.execute_query(
                abono_query, (monto, id_reserva, id_medio_pago), fetch=False
            )
            ...
            total_abonado = float(
                result[0][1]
            )  # Este total ya incluye el abono que acabamos de insertar
            ...
            """
            self.db_manager.execute_query(
                update_query, (nuevo_saldo_pendiente, id_reserva), fetch=False
            )
            ...
            if nuevo_saldo_pendiente <= 0:
                ...
                messagebox.showinfo(
                    "¡Reserva pagada!",
                    "¡Felicidades! Tu reserva ha sido completamente pagada.",
                )
            ...
            if metodo == "Efectivo":
                ...
                messagebox.showinfo(
                    "Abono procesado",
                    f"Abono de ${monto:,.0f} procesado exitosamente con {metodo}.",
                )
    ...
    def _simular_pasarela_pago(self, id_reserva, monto, metodo):
        ...
        progress_label = ctk.CTkLabel(
            win, text="Procesando pago...", font=("Arial", 12)
        )
        ...
        def procesar_pago():
            ...
            progress_label.configure(
                text="✅ Pago procesado exitosamente", text_color="#00FF99"
            )
            win.update()
            time.sleep(1)
    ...
    def _build_tab_perfil(self, parent):
        ...
        def guardar():
            try:
                ...
                if ok:
                    messagebox.showinfo(
                        "Éxito", "Perfil actualizado correctamente (ambas bases)"
                    )
                else:
                    messagebox.showerror(
                        "Error", "No se pudo actualizar el perfil en ambas bases"
                    )
            except Exception as exc:
    ...
    def _build_tab_abonos(self, parent):
        ...
        frame = ctk.CTkFrame(parent)
        frame.pack(expand=True, fill="both", padx=10, pady=10)
        ctk.CTkLabel(frame, text="Realizar Abonos", font=("Arial", 18, "bold")).pack(
            pady=10
        )
        ...
        ctk.CTkLabel(
            info_frame,
            text="• El primer abono debe ser al menos el 30% del valor total",
            font=("Arial", 11),
        ).pack(pady=2)
        ctk.CTkLabel(
            info_frame,
            text="• Los siguientes abonos pueden ser de cualquier valor",
            font=("Arial", 11),
        ).pack(pady=2)
        ctk.CTkLabel(
            info_frame,
        ...
        if reservas:
            for r in reservas:
                ...
                if saldo_real > 0:
                    ...
                    ctk.CTkLabel(
                        card, text=f"{modelo} ({placa})", font=("Arial", 14, "bold")
                    ).pack(anchor="w", padx=12, pady=(6, 0))
                    ctk.CTkLabel(
                        card,
                        text=f"Saldo pendiente: ${saldo_real:,.0f}",
                        font=("Arial", 12),
                        text_color="#B8860B",
                    ).pack(anchor="w", padx=12)
                    ctk.CTkLabel(
                        card,
                        text=f"Abonado: ${abonado:,.0f} ({porcentaje_abonado:.1f}%)",
                        font=("Arial", 12),
                    ).pack(anchor="w", padx=12)
                    if es_primer_abono:
                        ctk.CTkLabel(
                            card,
                            text=f"Mínimo 1er abono: ${monto_minimo:,.0f}",
                            font=("Arial", 11),
                            text_color="#C62828",
                        ).pack(anchor="w", padx=12)
                    ...
                    card.bind(
                        "<Button-1>",
                        lambda e, rid=id_reserva: self._seleccionar_abono_card(rid),
                    )
            ...
            if len(self.cards_abonos.winfo_children()) == 0:
                ctk.CTkLabel(
                    self.cards_abonos,
                    text="No tienes reservas pendientes de pago.",
                    font=("Arial", 13),
                    text_color="#C62828",
                ).pack(pady=20)
        else:
    ...
    def _realizar_abono(self):
        ...
        if not valor_result:
            messagebox.showerror(
                "Error", "No se pudo obtener información de la reserva"
            )
            return
        ...
        abonado_anterior = (
            abonos_result[0][0] if abonos_result and abonos_result[0] else 0
        )
        ...
        if abonado_anterior == 0:
            ...
            if monto_float < monto_minimo:
                messagebox.showwarning(
                    "Error",
                    f"El primer abono debe ser al menos el 30% del valor total (${monto_minimo:,.0f})",
                )
                return
        ...
        if monto_float > saldo_pendiente_float:
            messagebox.showwarning(
                "Error",
                f"El monto excede el saldo pendiente (${saldo_pendiente_float:,.0f})",
            )
            return
    ...
    def _build_tab_crear_reserva(self, parent):
        ...
        frame.pack(expand=True, fill="both", padx=10, pady=10)
        ctk.CTkLabel(
            frame, text="Crear nueva reserva", font=("Arial", 20, "bold")
        ).pack(pady=10)
        ...
        # Fecha y hora salida
        ctk.CTkLabel(card, text="Fecha y hora salida", font=("Arial", 12)).pack(
            anchor="w", pady=(10, 0), padx=12
        )
        ...
        salida_ampm_cb.pack(side="left", padx=2)
        # Fecha y hora entrada
        ctk.CTkLabel(card, text="Fecha y hora entrada", font=("Arial", 12)).pack(
            anchor="w", pady=(10, 0), padx=12
        )
        ...
        entrada_ampm_cb.pack(side="left", padx=2)
        # Seguro
        ctk.CTkLabel(card, text="Seguro", font=("Arial", 12)).pack(
            anchor="w", pady=(10, 0), padx=12
        )
        seguros = self.db_manager.execute_query(
            "SELECT id_seguro, descripcion, costo FROM Seguro_alquiler"
        ...
        if seguros:
            seguro_menu = tk.OptionMenu(
                card, seguro_var, *[f"{s[1]} (${s[2]})" for s in seguros]
            )
        ...
        total_label = ctk.CTkLabel(
            card, text="Total: $0", font=("Arial", 13, "bold"), text_color="#1976D2"
        )
        total_label.pack(pady=8, padx=12)
        abono_label = ctk.CTkLabel(
            card,
            text="Abono mínimo (30%): $0",
            font=("Arial", 12),
            text_color="#FFD700",
        )
        abono_label.pack(pady=4, padx=12)
        # Campo de abono inicial
        ctk.CTkLabel(card, text="Abono inicial ($)", font=("Arial", 12)).pack(
            anchor="w", pady=(10, 0), padx=12
        )
        ...
        def actualizar_total(*_):
            try:
                ...
                vehiculo_query = """
                    ...
                    WHERE v.placa = %s
                """
                vehiculo_result = self.db_manager.execute_query(
                    vehiculo_query, (placa,)
                )
        ...
        def crear_reserva():
            ...
            try:
                ...
                if not vehiculo_result:
                    messagebox.showerror(
                        "Error", "No se pudo obtener la tarifa del vehículo"
                    )
                    return
    ...
    def _build_tab_reservas(self, parent):
        ...
        if reservas:
            for r in reservas:
                listbox.insert(
                    "end", f"ID: {r[0]} | Cliente: {r[1]} | Vehículo: {r[2]}"
                )
        else:
...
¿Desea continuar con la actualización?"""
            ...
            """
            self.db_manager.execute_query(
                reserva_query, (nuevo_saldo, id_reserva), fetch=False
            )
