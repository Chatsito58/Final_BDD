...
class ClienteView(QMainWindow):
    def __init__(self, user_data, db_manager, auth_manager, on_logout):
        ...
        ui_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "ui", "client_view.ui"
        )
        loadUi(ui_path, self)
    ...
    def _setup_reservas_tab(self):
        ...
        self.reservas_list.itemSelectionChanged.connect(
            self._on_reserva_selection_changed
        )
        self._cargar_reservas_cliente()
    ...
    def _on_reserva_selection_changed(self):
        ...
        if selected_items:
            ...
            self._selected_reserva_id = int(
                item_text.split(":")[0].replace("Reserva ", "").strip()
            )
        else:
            self._selected_reserva_id = None
    ...
    def _cargar_reservas_cliente(self):
        ...
        for r in reservas:
            ...
            self.reservas_list.addItem(
                f"Reserva {id_reserva}: {modelo} ({placa}) - Salida: {salida} - Estado: {estado}"
            )
    ...
    def _cancelar_reserva(self):
        ...
        try:
            ...
            alquiler_id_query = (
                "SELECT id_alquiler FROM Reserva_alquiler WHERE id_reserva = %s"
            )
            alquiler_id = self.db_manager.execute_query(
                alquiler_id_query, (self._selected_reserva_id,)
            )[0][0]
            ...
            update_vehiculo_query = (
                "UPDATE Vehiculo SET id_estado_vehiculo = 1 WHERE placa = %s"
            )
    ...
    def _editar_reserva(self):
        ...
        reserva_data = self.db_manager.execute_query(
            query, (self._selected_reserva_id,)
        )

        if not reserva_data:
            QMessageBox.critical(
                self, "Error", "No se encontraron datos para la reserva seleccionada."
            )
            return
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Editar Fechas y Horas de Reserva")
        ...
        salida_datetime_edit.setDateTime(
            QDateTime.fromString(str(salida_actual), "yyyy-MM-dd HH:mm:ss")
        )
        ...
        entrada_datetime_edit.setDateTime(
            QDateTime.fromString(str(entrada_actual), "yyyy-MM-dd HH:mm:ss")
        )
        ...
        save_button.clicked.connect(
            lambda: self._guardar_edicion_reserva(
                dialog,
                salida_datetime_edit.dateTime().toString("yyyy-MM-dd HH:mm:ss"),
                entrada_datetime_edit.dateTime().toString("yyyy-MM-dd HH:mm:ss"),
                id_vehiculo,
                id_seguro,
                id_descuento,
                valor_actual,
        ...
        try:
            ...
            if nueva_entrada <= nueva_salida:
                QMessageBox.warning(
                    self,
                    "Error",
                    "La fecha de entrada debe ser posterior a la de salida.",
                )
                return
            ...
            if id_seguro:
                ...
                seguro_result = self.db_manager.execute_query(
                    seguro_query, (id_seguro,)
                )
                if seguro_result:
            ...
            if id_descuento:
                descuento_query = (
                    "SELECT valor FROM Descuento_alquiler WHERE id_descuento = %s"
                )
                descuento_result = self.db_manager.execute_query(
                    descuento_query, (id_descuento,)
                )
                if descuento_result:
            ...
            self.db_manager.update(
                update_alquiler_query,
                (
                    nueva_salida_str,
                    nueva_entrada_str,
                    nuevo_valor,
                    self._selected_reserva_id,
                ),
            )

            # Obtener el abono actual de la reserva
            abono_actual_query = (
                "SELECT abono FROM Reserva_alquiler WHERE id_reserva = %s"
            )
            abono_actual_result = self.db_manager.execute_query(
                abono_actual_query, (self._selected_reserva_id,)
            )
            abono_actual = (
                float(abono_actual_result[0][0]) if abono_actual_result else 0
            )
            ...
            update_reserva_saldo_query = (
                "UPDATE Reserva_alquiler SET saldo_pendiente = %s WHERE id_reserva = %s"
            )
            self.db_manager.update(
                update_reserva_saldo_query,
                (nuevo_saldo_pendiente, self._selected_reserva_id),
            )
            ...
            QMessageBox.critical(
                self, "Error", f"No se pudo actualizar la reserva: {e}"
            )
    ...
    def _cargar_vehiculos_disponibles(self):
        ...
        for v in self.vehiculos_data:
            self.vehiculo_combo.addItem(
                f"{v[0]} - {v[1]} {v[2]} ({v[3]}) - ${v[4]:,.0f}/día"
            )
    ...
    def _recalcular_total_reserva(self):
        ...
        try:
            ...
            vehiculo_info = next(
                (v for v in self.vehiculos_data if v[0] == placa), None
            )
            ...
            if selected_seguro_text != "Ninguno":
                seguro_info = next(
                    (
                        s
                        for s in self.seguros_data
                        if f"{s[1]} (${float(s[2]):,.0f})" == selected_seguro_text
                    ),
                    None,
                )
                if seguro_info:
                    seguro_costo = float(seguro_info[2])

            # Calcular descuento
            id_descuento, desc_text, desc_val = self._obtener_descuento_activo(
                fecha_salida, fecha_entrada
            )
            if id_descuento:
                self.descuento_label.setText(
                    f"Descuento Aplicado: {desc_text} (-${float(desc_val):,.0f})"
                )
            else:
                self.descuento_label.setText("Descuento Aplicado: Ninguno")
    ...
    def _guardar_reserva(self):
        try:
            ...
            fecha_hora_salida = self.salida_datetime.dateTime().toString(
                "yyyy-MM-dd HH:mm:ss"
            )
            fecha_hora_entrada = self.entrada_datetime.dateTime().toString(
                "yyyy-MM-dd HH:mm:ss"
            )
            ...
            if dt_salida < datetime.now():
                QMessageBox.warning(
                    self, "Error", "La fecha de salida no puede ser en el pasado."
                )
                return
            ...
            if not abono_str:
                QMessageBox.warning(
                    self, "Error", "Ingrese el monto del abono inicial."
                )
                return
            ...
            abono_min_str = (
                self.abono_min_label.text()
                .replace("Abono Mínimo (30%): $", "")
                .replace(",", "")
            )
            ...
            if not id_alquiler:
                QMessageBox.critical(
                    self, "Error", "No se pudo obtener el ID del alquiler."
                )
                return

            # Cambiar estado del vehículo a reservado (estado 2)
            update_vehiculo_query = (
                "UPDATE Vehiculo SET id_estado_vehiculo = 2 WHERE placa = %s"
            )
            self.db_manager.update(update_vehiculo_query, (placa,))
            ...
            if not id_reserva:
                QMessageBox.critical(
                    self, "Error", "No se pudo obtener el ID de la reserva."
                )
                return

            # Insertar abono inicial
            id_medio_pago = {"Efectivo": 1, "Tarjeta": 2, "Transferencia": 3}.get(
                metodo_pago, 1
            )
            ...
            self.db_manager.insert(
                abono_inicial_query, (abono, id_reserva, id_medio_pago)
            )
            ...
            if metodo_pago == "Efectivo":
                QMessageBox.information(
                    self,
                    "Reserva Registrada",
                    "Su reserva ha sido registrada. Por favor, diríjase a la sucursal para realizar el pago en efectivo.",
                )
            else:
    ...
    def _simular_pasarela_pago(self, id_reserva, monto, metodo):
        dialog = QDialog(self)
        dialog.setWindowTitle(f"Pasarela de Pago Simulada - {metodo}")
        layout = QVBoxLayout()
        ...
        QTimer.singleShot(
            2000,
            lambda: self._finalizar_simulacion_pago(dialog, id_reserva, monto, metodo),
        )
    ...
    def _finalizar_simulacion_pago(self, dialog, id_reserva, monto, metodo):
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Pago Exitoso",
                f"El pago de ${monto:,.0f} ha sido procesado exitosamente con {metodo}. Su reserva ha sido aprobada.",
            )
            self._cargar_reservas_cliente()
        except Exception as e:
    ...
    def _setup_abonos_tab(self):
        ...
        self.abonos_reservas_list.itemSelectionChanged.connect(
            self._on_abono_reserva_selection_changed
        )
    ...
    def _cargar_reservas_pendientes_abono(self):
        ...
        for r in reservas:
            ...
            self.abonos_reservas_list.addItem(
                f"Reserva {id_reserva}: {modelo} ({placa}) - Saldo: ${float(saldo_pendiente):,.0f} / Total: ${float(valor_total):,.0f}"
            )
    ...
    def _realizar_abono(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Error",
                f"Monto inválido: '{monto_str}'. Ingrese un número válido.",
            )
            return
        ...
        saldo_query = (
            "SELECT saldo_pendiente FROM Reserva_alquiler WHERE id_reserva = %s"
        )
        resultado_saldo = self.db_manager.execute_query(
            saldo_query, (self._selected_reserva_id,)
        )
        if not resultado_saldo:
            QMessageBox.critical(
                self, "Error", "No se pudo obtener el saldo de la reserva."
            )
            return
        ...
        if monto > saldo_actual:
            QMessageBox.warning(
                self,
                "Error",
                f"El monto a abonar (${monto:,.0f}) no puede ser mayor al saldo pendiente (${saldo_actual:,.0f}).",
            )
            return
        ...
        try:
            ...
            self.db_manager.insert(
                insert_abono_query, (monto, self._selected_reserva_id, id_medio_pago)
            )
            ...
            self.db_manager.update(
                update_reserva_query, (nuevo_saldo, self._selected_reserva_id)
            )
            ...
            if nuevo_saldo <= 0:
                ...
                self.db_manager.update(
                    update_estado_query, (self._selected_reserva_id,)
                )
                QMessageBox.information(
                    self, "Reserva Pagada", "¡La reserva ha sido completamente pagada!"
                )
            ...
            if metodo_pago == "Efectivo":
                QMessageBox.information(
                    self,
                    "Abono Registrado",
                    "Su abono ha sido registrado. Por favor, diríjase a la sucursal para realizar el pago en efectivo.",
                )
            else:
                self._simular_pasarela_pago(
                    self._selected_reserva_id, monto, metodo_pago
                )
    ...
    def _guardar_perfil(self):
        ...
        try:
            ...
            self.db_manager.update(
                query, (nombre, telefono, direccion, correo, id_cliente)
            )
            ...
            self.user_data["usuario"] = (
                nombre  # Asumiendo que el nombre es el usuario para el mensaje de bienvenida
            )
    ...
    def _cambiar_contrasena(self):
        ...
        if nueva_pass != confirmar_pass:
            QMessageBox.warning(
                self, "Error", "La nueva contraseña y la confirmación no coinciden."
            )
            return
        ...
        try:
            ...
            if auth_result is True:
                QMessageBox.information(
                    self, "Éxito", "Contraseña cambiada correctamente."
                )
